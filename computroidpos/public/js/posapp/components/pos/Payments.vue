          ...
          >
            ...
            >
              <v-btn
                ...
                @keydown="handleKeyPress(payment.mode_of_payment, payment.idx, $event)"
                >
...
export default {
  ...
  methods: {
    ...
    handleKeyPress(mode, idx, event) {
      switch (event.key) {
        case "F10":
          // Check if the key is F10 and the mode is 'UPI'
          if (mode === 'UPI') {
            event.preventDefault();
            this.$refs.vBtnContent[idx - 1].click();
          }
          break;
        case "C":
        case "c":
          // Handle "C" or "c" key press (Cash) based on idx
          this.handleCashKeyPress(idx);
          break;
        case "R":
        case "r":
          // Handle "R" or "r" key press (Credit Card) based on idx
          this.handleCreditCardKeyPress(idx);
          break;
        case "U":
        case "u":
          // Handle "U" or "u" key press (UPI) based on idx
          this.handleUpiKeyPress(idx);
          break;
        default:
    ...
    load_print_page() {
      ...
      printWindow.addEventListener(
        ...
        function () {
          printWindow.print();
          printWindow.close(); //Netmanthan:Jawahar
