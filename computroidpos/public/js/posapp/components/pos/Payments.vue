        ...
        switch (mode) {
          case 'Cash':
            if (event.key === 'C' || event.key === 'c') {
              this.set_full_amount(this.getPaymentIdx('Cash'));
            }
            break;
          case 'Credit Card':
            if (event.key === 'R' || event.key === 'r') {
              this.set_full_amount(this.getPaymentIdx('Credit Card'));
            }
            break;
          case 'UPI':
            if (event.key === 'U' || event.key === 'u') {
              this.set_full_amount(this.getPaymentIdx('UPI'));
            }
            break;
          default:
            // Handle any other keys or cases here
            break;
        }
      },
      getPaymentIdx(mode) {
        // Retrieve the idx for the given payment mode
        return this.payments.find((payment) => payment.mode_of_payment === mode)?.idx;
      },
      back_to_invoice() {
